...
export default function EditarConcursoPage() {
  ...
  const [salarioNivel, setSalarioNivel] = useState("");
    ...
    async function carregarConcurso() {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Se tiver locais, setar o último para o select
          if (data.locais && data.locais.length > 0) {
            const ultimoLocal = data.locais[data.locais.length - 1];
            const [cidade, estado] = ultimoLocal.split(" - ");
            if (estado) {
              setEstadoSelecionado(estado);
              setCidadeSelecionada(ultimoLocal);
            }
          }
        } else {
  ...
  const adicionarNivel = () => {
    ...
    const novoNivelInfo: NivelInfo = {
      ...
      vagas: vagasNivel || 0,
      salario: salarioNivel || "A definir",
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(docRef, {
        ...
        stats: {
          vagas: totalVagas,
          materias: formData.niveis.length,
          topicos: 0,
          horas: 0,
        },
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        ...
        />
        <p className="text-gray-500">Carregando concurso...</p>
      </div>
    );
  }
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="inline-flex items-center gap-2 bg-white/20 px-4 py-2 rounded-full mb-4">
              ...
              <span className="text-sm font-medium">Editar Concurso</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black mb-2">
              {formData.nome}
            </h1>
            <p className="text-purple-200 text-lg max-w-2xl">
              Edite as informações do concurso
            </p>
          </div>
          <button
            onClick={() => router.back()}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            type="text"
                            value={salarioNivel}
                            onChange={(e) => setSalarioNivel(e.target.value)}
                            className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all outline-none"
